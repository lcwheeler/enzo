...
class Pathway(object):
    ...
    def evolve(self, params, optimum1, target_index, total, constraint = 0.1, optimum_tolerance = 0.01, iterations=10000, stop=True):
        ...
        for i in range(iterations):
            ...
            metric_current_1 = SS_values_current[target_index]/sum(SS_values_current) 
            ...
            if all(i > 0 for i in SS_values) and np.sum(SS_values) > (total - constraint*total) and np.sum(SS_values) < (total + constraint*total):
                # Here I have hard-coded the metric for comparison as the ratio of interest, 
                # should make it a more flexible argument, but difficult to generalize
                # currently only the target floating species can be entered as argument
                metric_1 = SS_values[target_index]/sum(SS_values)  
    ...
    def plot_ss(self):
        ...
        print("The ratio of the target species to all others species is "+ str(SS_values[13]/sum(SS_values)))
